...
SEEN_FILE        = "seen_jobs.txt"          # Cache of all processed job IDs
...
def fetch_all_gh_companies() -> list:
    """
    Load Greenhouse company tokens from companies.txt,
    then supplement with SEED_TOKENS as fallback.
    """

    seen = set()
    tokens = set()

    def add_tokens(slugs):
        for s in slugs:
            slug = s.strip().lower()
    ...
    # ── Step 1: LOAD FROM FILE (PRIMARY SOURCE) ───────────────────────────
    try:
        with open("companies.txt", "r", encoding="utf-8") as f:
            text = f.read()
            if text:
                companies = {item.strip() for item in text.split(",") if item.strip()}
                add_tokens(companies)
                print(f"{len(tokens)} company names extracted from companies.txt")
            else:
                print("companies.txt empty")
                tokens = set()
    except FileNotFoundError:
        print("companies.txt not found")
        tokens = set()

    # ── Step 2: ADD SEED TOKENS (FALLBACK / SUPPLEMENT) ───────────────────
    add_tokens(SEED_TOKENS)

    print(f"Total companies to scan: {len(tokens)} "
          f"(file + seed fallback combined)")
...
def is_relevant(job: dict) -> tuple:
    ...
    in_us_state = any(state in location for state in US_STATES)
    in_non_us_country = any(country in location for country in NON_US_INDICATORS) 
    
    if location:
        if not in_us_state or in_non_us_country:
            return False, "non_us"
